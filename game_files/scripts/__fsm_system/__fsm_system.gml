...
function StateMachine(states_array, states_anim_array=undefined) constructor {
	...
	static Update = function() {
		if (__current_state != undefined) {
			if (__current_state.step_func != undefined){
				__current_state.step_func()
			}
		}
		if (__current_state != undefined) {
			if (__current_state.anim_func != undefined){
				__current_state.anim_func();
			}
		}
		if (__any_state__ != undefined) {
			if (__any_state__.any_state_func != undefined){
				__any_state__.any_state_func();
			}
		}
	}
	...
	/// @desc This function returns if the specified state exists
	/// @func StateExists()
	/// @param {String} _state The name of the state to check
	/// @returns {Bool}
	static StateExists = function(_state){
		return variable_struct_exists(self, _state);
	}
}
